...
{	
	 ...
	 * 
	 * @post Angle of this projectile gets set to the angle of the given worm
	 * 			| new.getAngle() == worm.getAngle()
	 */
	public Projectile(Worm worm) throws IllegalArgumentException
	{
		this.worm = worm;
		this.setAngle(worm.getAngle());
		if (worm.isValidPosition(worm.getPosX() + Math.cos(this.getAngle() * worm.getRadius()), worm.getPosY() + Math.sin(this.getAngle() * worm.getRadius())))
		{	
			this.setPosX(worm.getPosX() + Math.cos(this.getAngle() * this.getRadius()));
			this.setPosY(worm.getPosY() + Math.sin(this.getAngle() * this.getRadius()));
		}
		else 
	...
	/**
	 * Sets the angle of this projectile to the given angle
	 * 
	 * @param angle
	 * 		The given angle
	 * 
	 * @post
	 * 		The angle gets set to the new angle
	 * 			| new.getAngle() == angle
	 */
	@Raw @Model
	...
	/**
	 * Retrieves the angle of this projectile
	 * 
	 * @return
	 * 		Angle
	 */
	...
	/**
	 * Determines whether a worm has been hit or not
	 */
	...
	public double JumpTime(double delta)
	{
		...
		double jumpTime = 0;
		while (hitWorm == false && this.getWorld().isPassable(X, Y, this.getRadius()))
		{
